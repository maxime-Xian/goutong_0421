...
function runHealthCheck() {
    ...
    // Innovation: Relax memory check on macOS (Darwin) because os.freemem is often low due to cache
    const memCriticalThreshold = process.platform === 'darwin' ? 99 : 95;
    const memWarningThreshold = process.platform === 'darwin' ? 98 : 90;

    if (memPct > memCriticalThreshold) {
        checks.push({ name: 'memory', ok: false, status: `${memPct}% used`, severity: 'critical' });
        criticalErrors++;
    } else if (memPct > memWarningThreshold) {
        checks.push({ name: 'memory', ok: false, status: `${memPct}% used`, severity: 'warning' });
        warnings++;
    } else {
